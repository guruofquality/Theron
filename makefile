...
# "threads=[windows|boost]". Default is boost.
# "metrics=[true|false]". Default is false.
#
# Example command line: mingw32-make mode=debug threads=boost
#
# Theron can be built using two different threading implementations. One is a
# wrapper around Win32 threads and depends on windows.h. The other is a wrapper
# around boost::threads and depends on the Boost threading library.
#
...
# use of boost::threads instead. The rationale is that if you're using the
# makefile you're likely to be on a non-Windows platform. If you'd rather use
# Windows threads then you can specify this using "threads=windows" on the make
# command line (see below).
#
# To build with boost::threads you'll need an installation of Boost.
...
# is quite useful. Note that you only need the boost::thread component so
# if it helps you can probably disable the rest.
#
# Once Boost is installed and built, you should have a bunch of libraries to
# go with the headers that came with Boost. You'll need to update the
# BOOST_INCLUDE_PATH and BOOST_LIB_PATH variables below to point at your own
# Boost installation. You may also need to change the name of the built library
# to link against, specified with the BOOST_RELEASE_LIB and BOOST_DEBUG_LIB
# make variables; the names encode the build configurations.
#
...
#
# Boost variables - set these to reflect your own Boost installation!
# Change the boost include and library paths to match your own locations.
# Change the names of the release and debug libraries to match your boost build.
# The format is "boost-{library}-{compiler}-{threadmode}-{build}-{version}" where
#  {library} is "thread" for the boost::thread library.
#  {compiler} is "mgw44" for GCC 4.4.x within MinGW.
#  {threadmode} is "mt" for multi-threaded builds.
#  {build} is "s" for static linkage, "d" for debug builds.
#  {version} is "1_47" for the Boost 1.47 release.
# On Ubuntu, boost_thread seems to be enough; on Mac OS try boost_thread-mt
#

BOOST_INCLUDE_PATH = /usr/include/
BOOST_LIB_PATH = /usr/lib/
BOOST_RELEASE_LIB = boost_thread-mgw44-mt-s-1_47
BOOST_DEBUG_LIB = boost_thread-mgw44-mt-sd-1_47
...
# Use "threads=boost" to force boost::threads (default).
#

ifeq ($(threads),windows)
	CFLAGS += -DTHERON_USE_BOOST_THREADS=0
	BOOST_INCLUDE_FLAGS =
	BOOST_LIB_FLAGS =
else
	CFLAGS += -DTHERON_USE_BOOST_THREADS=1 -DBOOST_THREAD_BUILD_LIB=1
...
${UNITTESTS}: $(THERON_LIB) ${UNITTESTS_OBJECTS}
	$(CC) ${UNITTESTS_OBJECTS} $(THERON_LIB) -o ${UNITTESTS} ${BOOST_LIB_FLAGS}
...
${CREATINGANACTOR}: $(THERON_LIB) ${CREATINGANACTOR_OBJECTS}
	$(CC) ${CREATINGANACTOR_OBJECTS} $(THERON_LIB) -o ${CREATINGANACTOR} ${BOOST_LIB_FLAGS}
...
${DEFAULTMESSAGEHANDLER}: $(THERON_LIB) ${DEFAULTMESSAGEHANDLER_OBJECTS}
	$(CC) ${DEFAULTMESSAGEHANDLER_OBJECTS} $(THERON_LIB) -o ${DEFAULTMESSAGEHANDLER} ${BOOST_LIB_FLAGS}
...
${DYNAMICHANDLERREGISTRATION}: $(THERON_LIB) ${DYNAMICHANDLERREGISTRATION_OBJECTS}
	$(CC) ${DYNAMICHANDLERREGISTRATION_OBJECTS} $(THERON_LIB) -o ${DYNAMICHANDLERREGISTRATION} ${BOOST_LIB_FLAGS}
...
${HANDLINGMESSAGES}: $(THERON_LIB) ${HANDLINGMESSAGES_OBJECTS}
	$(CC) ${HANDLINGMESSAGES_OBJECTS} $(THERON_LIB) -o ${HANDLINGMESSAGES} ${BOOST_LIB_FLAGS}
...
${RECEIVER}: $(THERON_LIB) ${RECEIVER_OBJECTS}
	$(CC) ${RECEIVER_OBJECTS} $(THERON_LIB) -o ${RECEIVER} ${BOOST_LIB_FLAGS}
...
${INITIALIZINGANACTOR}: $(THERON_LIB) ${INITIALIZINGANACTOR_OBJECTS}
	$(CC) ${INITIALIZINGANACTOR_OBJECTS} $(THERON_LIB) -o ${INITIALIZINGANACTOR} ${BOOST_LIB_FLAGS}
...
${INITIALIZINGTHEFRAMEWORK}: $(THERON_LIB) ${INITIALIZINGTHEFRAMEWORK_OBJECTS}
	$(CC) ${INITIALIZINGTHEFRAMEWORK_OBJECTS} $(THERON_LIB) -o ${INITIALIZINGTHEFRAMEWORK} ${BOOST_LIB_FLAGS}
...
${DERIVEDMESSAGETYPES}: $(THERON_LIB) ${DERIVEDMESSAGETYPES_OBJECTS}
	$(CC) ${DERIVEDMESSAGETYPES_OBJECTS} $(THERON_LIB) -o ${DERIVEDMESSAGETYPES} ${BOOST_LIB_FLAGS}
...
${REGISTERINGMESSAGES}: $(THERON_LIB) ${REGISTERINGMESSAGES_OBJECTS}
	$(CC) ${REGISTERINGMESSAGES_OBJECTS} $(THERON_LIB) -o ${REGISTERINGMESSAGES} ${BOOST_LIB_FLAGS} -fno-rtti
...
${ALIGNINGACTORS}: $(THERON_LIB) ${ALIGNINGACTORS_OBJECTS}
	$(CC) ${ALIGNINGACTORS_OBJECTS} $(THERON_LIB) -o ${ALIGNINGACTORS} ${BOOST_LIB_FLAGS}
...
${ALIGNINGMESSAGES}: $(THERON_LIB) ${ALIGNINGMESSAGES_OBJECTS}
	$(CC) ${ALIGNINGMESSAGES_OBJECTS} $(THERON_LIB) -o ${ALIGNINGMESSAGES} ${BOOST_LIB_FLAGS}
...
${SENDINGMESSAGES}: $(THERON_LIB) ${SENDINGMESSAGES_OBJECTS}
	$(CC) ${SENDINGMESSAGES_OBJECTS} $(THERON_LIB) -o ${SENDINGMESSAGES} ${BOOST_LIB_FLAGS}
...
${TERMINATINGTHEFRAMEWORK}: $(THERON_LIB) ${TERMINATINGTHEFRAMEWORK_OBJECTS}
	$(CC) ${TERMINATINGTHEFRAMEWORK_OBJECTS} $(THERON_LIB) -o ${TERMINATINGTHEFRAMEWORK} ${BOOST_LIB_FLAGS}
...
${ACTORREFERENCES}: $(THERON_LIB) ${ACTORREFERENCES_OBJECTS}
	$(CC) ${ACTORREFERENCES_OBJECTS} $(THERON_LIB) -o ${ACTORREFERENCES} ${BOOST_LIB_FLAGS}
...
${CUSTOMALLOCATOR}: $(THERON_LIB) ${CUSTOMALLOCATOR_OBJECTS}
	$(CC) ${CUSTOMALLOCATOR_OBJECTS} $(THERON_LIB) -o ${CUSTOMALLOCATOR} ${BOOST_LIB_FLAGS}
...
${MULTIPLEFRAMEWORKS}: $(THERON_LIB) ${MULTIPLEFRAMEWORKS_OBJECTS}
	$(CC) ${MULTIPLEFRAMEWORKS_OBJECTS} $(THERON_LIB) -o ${MULTIPLEFRAMEWORKS} ${BOOST_LIB_FLAGS}
...
${MEASURINGTHREADUTILIZATION}: $(THERON_LIB) ${MEASURINGTHREADUTILIZATION_OBJECTS}
	$(CC) ${MEASURINGTHREADUTILIZATION_OBJECTS} $(THERON_LIB) -o ${MEASURINGTHREADUTILIZATION} ${BOOST_LIB_FLAGS}
...
${SETTINGTHETHREADCOUNT}: $(THERON_LIB) ${SETTINGTHETHREADCOUNT_OBJECTS}
	$(CC) ${SETTINGTHETHREADCOUNT_OBJECTS} $(THERON_LIB) -o ${SETTINGTHETHREADCOUNT} ${BOOST_LIB_FLAGS}
...
${NONTRIVIALMESSAGES}: $(THERON_LIB) ${NONTRIVIALMESSAGES_OBJECTS}
	$(CC) ${NONTRIVIALMESSAGES_OBJECTS} $(THERON_LIB) -o ${NONTRIVIALMESSAGES} ${BOOST_LIB_FLAGS}
...
${ENVELOPEMESSAGES}: $(THERON_LIB) ${ENVELOPEMESSAGES_OBJECTS}
	$(CC) ${ENVELOPEMESSAGES_OBJECTS} $(THERON_LIB) -o ${ENVELOPEMESSAGES} ${BOOST_LIB_FLAGS}
...
${FILEREADER}: $(THERON_LIB) ${FILEREADER_OBJECTS}
	$(CC) ${FILEREADER_OBJECTS} $(THERON_LIB) -o ${FILEREADER} ${BOOST_LIB_FLAGS}
...
${MANAGINGTHETHREADPOOL}: $(THERON_LIB) ${MANAGINGTHETHREADPOOL_OBJECTS}
	$(CC) ${MANAGINGTHETHREADPOOL_OBJECTS} $(THERON_LIB) -o ${MANAGINGTHETHREADPOOL} ${BOOST_LIB_FLAGS}
...
${FALLBACKHANDLER}: $(THERON_LIB) ${FALLBACKHANDLER_OBJECTS}
	$(CC) ${FALLBACKHANDLER_OBJECTS} $(THERON_LIB) -o ${FALLBACKHANDLER} ${BOOST_LIB_FLAGS}
...
${THREADRING}: $(THERON_LIB) ${THREADRING_OBJECTS}
	$(CC) ${THREADRING_OBJECTS} $(THERON_LIB) -o ${THREADRING} ${BOOST_LIB_FLAGS}
...
${PARALLELTHREADRING}: $(THERON_LIB) ${PARALLELTHREADRING_OBJECTS}
	$(CC) ${PARALLELTHREADRING_OBJECTS} $(THERON_LIB) -o ${PARALLELTHREADRING} ${BOOST_LIB_FLAGS}
...
${COUNTINGACTOR}: $(THERON_LIB) ${COUNTINGACTOR_OBJECTS}
	$(CC) ${COUNTINGACTOR_OBJECTS} $(THERON_LIB) -o ${COUNTINGACTOR} ${BOOST_LIB_FLAGS}
...
${PRODUCERCONSUMER}: $(THERON_LIB) ${PRODUCERCONSUMER_OBJECTS}
	$(CC) ${PRODUCERCONSUMER_OBJECTS} $(THERON_LIB) -o ${PRODUCERCONSUMER} ${BOOST_LIB_FLAGS}
...
${ACTOROVERHEAD}: $(THERON_LIB) ${ACTOROVERHEAD_OBJECTS}
	$(CC) ${ACTOROVERHEAD_OBJECTS} $(THERON_LIB) -o ${ACTOROVERHEAD} ${BOOST_LIB_FLAGS}
...
${PINGPONG}: $(THERON_LIB) ${PINGPONG_OBJECTS}
	$(CC) ${PINGPONG_OBJECTS} $(THERON_LIB) -o ${PINGPONG} ${BOOST_LIB_FLAGS}
