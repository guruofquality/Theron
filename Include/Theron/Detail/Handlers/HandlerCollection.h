...
THERON_FORCEINLINE bool HandlerCollection::Remove(void (ActorType::*handler)(const ValueType &message, const Address from))
{
    ...
    const typename MessageHandlerList::Iterator handlersBegin(mHandlers.Begin());
    const typename MessageHandlerList::Iterator handlersEnd(mHandlers.End());

    for (typename MessageHandlerList::Iterator handlers(handlersBegin); handlers != handlersEnd; ++handlers)
    {
    ...
    const typename MessageHandlerList::Iterator newHandlersBegin(mNewHandlers.Begin());
    const typename MessageHandlerList::Iterator newHandlersEnd(mNewHandlers.End());

    for (typename MessageHandlerList::Iterator handlers(newHandlersBegin); handlers != newHandlersEnd; ++handlers)
    {
        IMessageHandler *const messageHandler(*handlers);
...
THERON_FORCEINLINE bool HandlerCollection::Handle(Actor *const actor, const IMessage *const message)
{
    ...
    const MessageHandlerList::Iterator handlersBegin(mHandlers.Begin());
    const MessageHandlerList::Iterator handlersEnd(mHandlers.End());

    for (MessageHandlerList::Iterator handlers(handlersBegin); handlers != handlersEnd; ++handlers)
    {
        const IMessageHandler *const handler(*handlers);
        handled |= handler->Handle(actor, message);
    }
