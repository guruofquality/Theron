...
inline void BlockingQueue::InitializeSharedContext(ContextType *const context)
{
    ...
    // The shared context holds a lock on the shared queue but it is initially unlocked.
    // It's only locked explicitly when accessing the shared queue.
    ...
    const bool localThread)
{
    ...
    // The shared context also doesn't have a local queue.
