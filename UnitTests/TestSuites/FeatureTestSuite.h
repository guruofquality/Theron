...
public:
    ...
    inline FeatureTestSuite()
    {
        ...
        TESTFRAMEWORK_REGISTER_TEST(SendMessagesToShortLivedActor);
        TESTFRAMEWORK_REGISTER_TEST(SetFallbackHandler);
        TESTFRAMEWORK_REGISTER_TEST(HandleUndeliveredMessageSentInFunction);
        TESTFRAMEWORK_REGISTER_TEST(HandleUndeliveredMessageSentInHandler);
    ...
    inline static void SendMessagesToShortLivedActor()
    {
        typedef Accumulator<Theron::uint32_t> UIntAccumulator;
        ...
        {
            Theron::ActorRef actor(framework.CreateActor<UIntReplier>());

            // Send the actor a series of messages.
            framework.Send(Theron::uint32_t(0), receiver.GetAddress(), actor.GetAddress());
            framework.Send(Theron::uint32_t(1), receiver.GetAddress(), actor.GetAddress());
            framework.Send(Theron::uint32_t(2), receiver.GetAddress(), actor.GetAddress());
            framework.Send(Theron::uint32_t(3), receiver.GetAddress(), actor.GetAddress());
            framework.Send(Theron::uint32_t(4), receiver.GetAddress(), actor.GetAddress());
            framework.Send(Theron::uint32_t(5), receiver.GetAddress(), actor.GetAddress());
            framework.Send(Theron::uint32_t(6), receiver.GetAddress(), actor.GetAddress());
            framework.Send(Theron::uint32_t(7), receiver.GetAddress(), actor.GetAddress());
        }

        // Check that we received the replies even though the actor became unreferenced.

        Theron::uint32_t count(8);
        while (count)
        {
            count -= receiver.Wait(8);
        }

        Check(accumulator.Pop() == 0, "Bad returned message");
        Check(accumulator.Pop() == 1, "Bad returned message");
        Check(accumulator.Pop() == 2, "Bad returned message");
        Check(accumulator.Pop() == 3, "Bad returned message");
        Check(accumulator.Pop() == 4, "Bad returned message");
        Check(accumulator.Pop() == 5, "Bad returned message");
        Check(accumulator.Pop() == 6, "Bad returned message");
        Check(accumulator.Pop() == 7, "Bad returned message");
    }
    ...
    inline static void HandleUndeliveredMessageSentInFunction()
    {
        ...
        Theron::Address lastGaspAddress;

        {
            Theron::ActorRef actor(framework.CreateActor<LastGasp>(receiver.GetAddress()));
    ...
    inline static void HandleUndeliveredMessageSentInHandler()
    {
        FallbackHandler fallbackHandler;
        Theron::Address signalerAddress;
        ...
        // Check that the undelivered message was handled by the registered fallback handler.
        Check(fallbackHandler.mAddress == signalerAddress, "Fallback handler failed");
    }
    ...
    inline static void HandleUndeliveredBlindMessageSentInFunction()
    {
        ...
        // Send a message to the stale address.
        framework.Send(Theron::uint32_t(42), receiver.GetAddress(), lastGaspAddress);

        // Check that the undelivered message was handled by the registered fallback handler.
        Check(fallbackHandler.mData != 0, "Blind fallback handler failed");
...
THERON_REGISTER_MESSAGE(UnitTests::FeatureTestSuite::IntVectorMessage);
